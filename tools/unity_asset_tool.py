...
# Компонент выдачи готовой сборки из ящика. Отличается от штатного Box тем,
# что разводит груз и обломки вскрытого ящика: рама BigMiner рождается внутри
...
def write_spawner_prefab(
    ...
    for p in parts:
        ...
        parts_yaml += (
            ...
            # Хозяин слота: рама/корпус или материнская плата. Спавнеру он
            # нужен, чтобы заранее знать нагрузку на каждую опору — плата
            # весит 1.0 и несёт 2.5, её тоже надо утяжелять, иначе она
            # отваливается вместе с процессором при тряске рамы.
            f"    hostPrefabName: {p.get('host', '')}\n"
        )
    if not parts_yaml:
        parts_yaml = "  []\n"
...
def write_crate_prefab(
    ...
    # Штатный Box заменяем на ReadyBuildBox: он гасит столкновение груза с
    # обломками вскрытого ящика. Рама BigMiner (10.54 в высоту) рождается
    # внутри шести обломков Part, и они выпихивают её вместе с видеокартами.
    # Увеличить ящик до вмещающего нельзя — нужен масштаб x2.45, это 11.03
    # в высоту, а портал висит на 8.
    tail = tail.replace(
        f"m_Script: {{fileID: 11500000, guid: {SCRIPT_GUIDS['Box']}, type: 3}}",
        f"m_Script: {{fileID: 11500000, guid: {READY_BOX_SCRIPT_GUID}, type: 3}}",
        1,
    )

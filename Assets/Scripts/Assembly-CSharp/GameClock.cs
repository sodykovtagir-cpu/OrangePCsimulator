...
public static class GameClock
{
	...
	/// <summary>
	/// Метка "сколько игровых минут прошло" — для сравнения без сборки строки.
	/// </summary>
	/// <remarks>
	...
	///
	/// Считать метку по СЕКУНДАМ здесь нельзя: игровая секунда проходит за
	/// одну шестидесятую реальной, то есть менялась бы почти каждый кадр и
	/// экономии не осталось бы вовсе. Игровая минута — это ровно секунда
	/// реального времени, что и даёт нужные 59 сэкономленных кадров из 60.
	/// </remarks>
	public static int Stamp { get { return Mathf.FloorToInt(SecondsOfDay / 60f); } }
}

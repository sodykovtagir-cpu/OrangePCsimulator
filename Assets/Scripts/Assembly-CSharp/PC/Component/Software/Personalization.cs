...
namespace PC.Component.Software
{
    public class Personalization : App
    {
        ...
        public void SetWallpaperFit(int mode)
        {
            ...
            if (fitDropdown != null)
                fitDropdown.SetValueWithoutNotify(Mathf.Clamp(mode, 0, fitLabels.Length - 1));
        }

        // Инициализация выпадающего списка режимов обоев. Сам UI ты создаёшь в
        // префабе и перетаскиваешь в поле fitDropdown в инспекторе (код только
        // наполняет список и подписывается на выбор).
        private void InitWallpaperFitDropdown()
        {
            ...
            fitDropdown.SetValueWithoutNotify(current);

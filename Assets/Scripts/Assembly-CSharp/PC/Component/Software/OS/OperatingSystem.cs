...
namespace PC.Component.Software.OS
{
    public class OperatingSystem : ComputerSystem
    {
        ...
        private void Update()
        {
            ...
            if (clockText != null)
            {
                // Часы показывают время до минут, но Update вызывается 60 раз
                // в секунду. Раньше строка собиралась каждый кадр:
                // интерполяция $"{...}" — это boxing, подстроки и склейка, то
                // есть мусор в куче на каждый кадр КАЖДОГО компьютера в мире.
                // Десяток включённых машин давал тысячи аллокаций в секунду, а
                // на слабом Android каждая сборка мусора — это видимый фриз.
                //
                // Теперь строка пересобирается только когда сменилась игровая
                // минута, и без интерполяции: из заранее нарезанных двух
                // символов на число.
                //
        ...
        // Игровая минута меняется раз в реальную секунду, то есть раз в
        // 60 кадров — запоминаем, что уже показано.
